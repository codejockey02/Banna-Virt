...
def pageFaultHandler(pageNumber, tlb, pageTable, physicalMemory):
    if int(pageNumber) < 256:
       ...
       # outputFile.write('Found page \"' + str(pageNumber) + '\" has data: <br>')
        print(physicalMemory[int(frameNumber)])
        #outputfile.write(physicalMemory[int(frameNumber)]+'<br>')
        print('in the backing store!\n')
        #outputFile.write('in the backing store<br>')

    else:
        print('Page \"' + pageNumber + '\" is out of bound!')
        #outputFile.write('Page \"' + pageNumber + '\" is out of bound!<br>')
        return
...
def updateTLB(pageNumber, frameNumber, tlb):
    ...
    #outputFile.write('Successfully update TLB with pageNumber: ' + str(pageNumber) + ', frameNumber: ' + str(frameNumber) + '!')
